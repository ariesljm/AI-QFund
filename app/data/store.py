...
def save_fund_features(features: dict, conn=None) -> None:
    ...
    写不变式：可重建表（含 fund_features）的写归 data/store（原在 repo/base，Q1 收敛）。
    conn 为 store 内部批量 seam（save_nav_batch 等惯例）；缺省时自开连接。
    """
    sql = 'INSERT OR REPLACE INTO fund_features (code, date, regime, hurst_60d, momentum_20d, calmar, downside_vol, capture_up, capture_down, drawdown_60d, reversal_20d, mom_5d, mom_60d, mom_250d, vol_20d, sharpe_60d, sortino_60d, ttr_60d, style_r2, rbsa_industry_1, rbsa_weight_1, rbsa_industry_2, rbsa_weight_2, rbsa_industry_3, rbsa_weight_3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    params = (features['code'], features['date'], features['regime'], features.get('hurst_60d'), features.get('momentum_20d'), features.get('calmar'), features.get('downside_vol'), features.get('capture_up'), features.get('capture_down'), features.get('drawdown_60d'), features.get('reversal_20d'), features.get('mom_5d'), features.get('mom_60d'), features.get('mom_250d'), features.get('vol_20d'), features.get('sharpe_60d'), features.get('sortino_60d'), features.get('ttr_60d'), features.get('style_r2', 0.0), features.get('rbsa_industry_1', ''), features.get('rbsa_weight_1', 0.0), features.get('rbsa_industry_2', ''), features.get('rbsa_weight_2', 0.0), features.get('rbsa_industry_3', ''), features.get('rbsa_weight_3', 0.0))
    if conn is not None:
...
def trim_fund_features(retention: int, conn=None) -> None:
    ...
    conn 为 store 内部批量 seam；缺省时自开连接。
    """
