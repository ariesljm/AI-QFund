...
def portfolio_series():
    ...
    if not funds or not min_date:
        return [], [], []
    rows = repo.get_nav_rows_for_codes([f["code"] for f in funds])
    nav_by_fund = {}
    date_set = set()
...
def period_returns(code):
    """计算基金多周期收益率及同期沪深300收益。"""
    rows = repo.get_nav_history(code, 250)
