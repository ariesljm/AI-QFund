...
def sector_selection_prompt_free(
    date_str: str,
    available: list[str],
    top_gainers: str,
    top_losers: str,
    ...
    """旧策略（llm_free）选赛道 prompt：LLM 从全清单自由选择（A/B 影子/回滚用）。"""
    lines = [
        f"你是专业的宏观分析师。基于 {date_str} 的多源数据，"
    ...
    lines += [
        "",
        "【否决权】",
        "若你认为清单中某赛道被新闻/政策证伪或产业逻辑破裂，放入 vetoed_sectors 并给出理由；",
        "否决的赛道不会进入推荐。",
        "",
        "输出以下 JSON（纯 JSON，勿用 markdown 代码块）：",
        "{",
        '  "recommended_sectors": ["行业1", "行业2", ...],',
        '  "risk_sectors": ["回避行业1", ...],',
        '  "vetoed_sectors": [{"sector": "否决的赛道", "reason": "否决理由"}],',
        '  "regime_label": "bullish/bearish/neutral",',
        '  "reasoning": "为什么选这些赛道，基于新闻中的产业动态/政策风向/资金流向；全部使用中文，描述大盘状态用「牛市/熊市/中性」，禁止出现英文 bullish/bearish/neutral"',
        "}",
    ]
