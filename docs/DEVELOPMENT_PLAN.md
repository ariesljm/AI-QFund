...
>
> ⚠️ **历史计划档案**：各 Phase 已全部完成（见「总体执行检查清单」）。计划中的目标函数、防线数、Web 形态等已随 R1 演进与迭代变化，**最新口径以 `docs/README.md` 与 `CONTEXT.md` 为准**。
...
| 单基历史净值 | `http://fund.eastmoney.com/pingzhongdata/{code}.js` | 单位净值、累计净值（JS var 格式，正则提取）|
...
## Phase 5：Web 展现层 (已实现，见 `app/web/`)

**目标**：只读仪表板，展示信号、持仓、进化规则。

**现状**：已实现为 FastAPI + Jinja2 **单页仪表板**（`app/web/app.py` + `dashboard.py` + `static/` + `templates/`，保留旧版 v1 模板），功能需求见 `docs/README.md` 第 8 节。
...
| **5** | Web 展现层 | ✅ | 单页仪表板（推荐/持仓/宏观/信号/质量度量）正常运行，数据实时刷新 |

---

## 领域口径补充（grilling 确认）

- **预测窗口 vs 持有周期**：模型预测"未来 20 交易日相对沪深300 的超额收益"是**买入信号**；"30 天"是产品层的**持有/监控周期**，非硬性离场期限（持仓由监控引擎三道防线决定离场）。
- **空推荐日**：LLM 显式判断当日无合适机会时，管线正常结束并记录"今日无推荐"（Web 展示原因）；仅 LLM 调用/解析失败才视为异常。
- **四类信号**：持有(HOLD)/加仓(BUY_MORE)/警惕(WARNING)/离场(EXIT)，仅面板展示不主动推送；加仓信号仅作建议提示，虚拟池收益仍按首次入场净值计算。
- **重复推荐**：允许同一基金多次推荐（记录推荐次数并展示），累计涨跌幅始终按首次推荐净值计算。
- **自我进化闭环**：LLM 洞察 + 排序权重自纠偏 + **每周模型自动重训** + **推荐质量度量**（IC / 20日超额胜率 / 累计超额曲线），度量反哺参数与洞察采纳（见 `.scratch/gap-closure/`）。
