...
class TestIsTradingDay:
    ...
    def test_fetch_trims_to_recent_two_years(self, monkeypatch):
        """解码后 1990 年至今全量 → 裁剪为最近两年。"""
        days = [f"{y}-{m:02d}-01" for y in range(1990, 2027) for m in (1, 6)]
        monkeypatch.setattr(trading_calendar, "_fetch_sina_calendar_text",
                            lambda: 'var datelist="fake";')
        monkeypatch.setattr(trading_calendar, "_decode_sina_calendar", lambda text: days)
...
class TestTradingDayLag:
    """滞后交易日数纯函数（净值停更打标 / 特征新鲜度共用单一来源）。"""

    def test_lag_counts_middle_days(self):
        days = {"2026-08-03", "2026-08-04", "2026-08-05", "2026-08-06"}
        assert trading_calendar.trading_day_lag("2026-08-03", "2026-08-06", days=days) == 3
        assert trading_calendar.trading_day_lag("2026-08-03", "2026-08-04", days=days) == 1

    def test_excludes_earlier_includes_later(self):
        days = {"2026-08-03", "2026-08-04", "2026-08-05"}
        # 不含 earlier（08-03），含 later（08-05）：计入 08-04 与 08-05
        assert trading_calendar.trading_day_lag("2026-08-03", "2026-08-05", days=days) == 2

    def test_earlier_equal_returns_zero(self):
        days = {"2026-08-03", "2026-08-04"}
        assert trading_calendar.trading_day_lag("2026-08-04", "2026-08-04", days=days) == 0
        assert trading_calendar.trading_day_lag("2026-08-05", "2026-08-04", days=days) == 0

    def test_no_days_returns_zero(self):
        assert trading_calendar.trading_day_lag("2026-08-03", "2026-08-06", days=set()) == 0

    def test_none_days_uses_meta_cache(self, monkeypatch):
        monkeypatch.setattr(trading_calendar, "_load_from_meta", lambda: None)
        assert trading_calendar.trading_day_lag("2026-08-03", "2026-08-06", days=None) == 0

    def test_matches_feature_freshness_semantics(self):
        """_feature_freshness 的原手工计数 == trading_day_lag（回归锁定）。"""
        days = {"2026-08-03", "2026-08-04", "2026-08-05", "2026-08-06"}
        feat_date, expected = "2026-08-04", "2026-08-06"
        manual = sum(1 for d in days if feat_date < d <= expected)
        assert trading_calendar.trading_day_lag(feat_date, expected, days=days) == manual
