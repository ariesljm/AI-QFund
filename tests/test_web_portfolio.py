...
class TestPortfolioSeries:
    ...
    def test_hs300_aligned(self, monkey_db):
        ...
        _, _, hs = webapp._portfolio_series()
...
class TestSharpeAndDrawdown:
    def test_max_drawdown(self):
        """最大回撤 = 累计收益曲线的最大峰谷回撤。"""
        assert webapp._max_drawdown([0, 10, 13, 10, 30]) == 3.0
        assert webapp._max_drawdown([0, -5, -8]) == 8.0
        assert webapp._max_drawdown([0, 5, 10]) == 0.0

    def test_sharpe_ratio_positive_for_up_trend(self):
        """上升趋势组合 → 正夏普；持平组合 → None（零波动）。"""
        s = webapp._sharpe_ratio([0, 10, 13, 10, 30])
        assert s is not None and s > 0
        assert webapp._sharpe_ratio([5, 5, 5]) is None
        assert webapp._sharpe_ratio([0]) is None

    def test_dual_svg_generated(self, monkey_db):
        """组合双线 SVG 正常生成且带 0% 基线。"""
        _, port, hs = webapp._portfolio_series()
        svg, hsvg, baseline = webapp._make_dual_svg(port, hs)
